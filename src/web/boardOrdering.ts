...
export function normalizePrioritiesInCurrentOrder<T extends BoardOrderItem>(items: T[]): T[] {
...
export function moveIntoStatus<T extends BoardOrderItem>(
  ...
  return normalizePrioritiesInCurrentOrder([
    ...remaining.slice(0, safeInsertAt),
    moved,
    ...remaining.slice(safeInsertAt),
  ]);
}
...
export function moveWithinStatus<T extends BoardOrderItem>(
  ...
  return normalizePrioritiesInCurrentOrder(
    statuses.flatMap((entryStatus) => (entryStatus === status ? movedStatusItems : grouped[entryStatus])),
  );
}
...
export function previewCrossStatusDragOver<T extends BoardOrderItem>(
  dragStartItems: T[],
  activeId: string,
  overId: string,
) {
  const active = dragStartItems.find((item) => item.id === activeId);
  const targetStatus = statusForDropTarget(dragStartItems, overId);
  if (!active || !targetStatus || active.status === targetStatus) return dragStartItems;

  const overItem = dragStartItems.find((item) => item.id === overId);
  return moveIntoStatus(dragStartItems, activeId, targetStatus, overItem?.id ?? null);
}

export function resolveDropTargetId<T extends BoardOrderItem>(
  dragStartItems: T[],
  activeId: string,
  overId: string | null,
  fallbackOverId: string | null,
) {
  if (overId && overId !== activeId) return overId;
  if (!fallbackOverId || fallbackOverId === activeId) return overId;

  const active = dragStartItems.find((item) => item.id === activeId);
  const fallbackStatus = statusForDropTarget(dragStartItems, fallbackOverId);
  if (!active || !fallbackStatus) return overId;

  return fallbackOverId;
}
