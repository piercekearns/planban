...
export function detachedBoardsRoot(): string {
  return join(defaultPlanbanRoot(), "detached");
}

export function detachedBoardCwd(repoId: string): string {
  return join(detachedBoardsRoot(), slugify(repoId));
}
