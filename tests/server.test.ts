...
test("duplicates, archives, restores, and deletes whole boards through the board API", async () => {
  await initializeProject({ cwd, title: "Server Test", repoId, updateAgents: false });
  await createCard({ cwd, title: "Alpha", status: "pending" });
  await initializeProject({ cwd: otherCwd, title: "Other Board", repoId: otherRepoId, updateAgents: false });
  const server = await startServer({ cwd, port: await freePort(), useVite: false });
  ...
  try {
    const duplicated = await jsonFetch<{ board: { repoId: string; title: string } }>(
      `${server.url}/api/boards/${repoId}/duplicate`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ repoId: `${repoId}-copy`, title: "Server Copy" }),
      },
    );
    assert.equal(duplicated.board.repoId, `${repoId}-copy`);
    assert.equal(duplicated.board.title, "Server Copy");
    const duplicateState = await jsonFetch<{ roadmap: { project: { title: string }; roadmapItems: Array<{ id: string }> } }>(
      `${server.url}/api/boards/${repoId}-copy/state`,
    );
    assert.equal(duplicateState.roadmap.project.title, "Server Copy");
    assert.deepEqual(duplicateState.roadmap.roadmapItems.map((item) => item.id), ["alpha"]);
    ...
    assert.deepEqual(active.boards.map((board) => board.repoId).sort(), [`${repoId}-copy`, otherRepoId].sort());
