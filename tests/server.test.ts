...
test("serves the built app and exposes state APIs", async () => {
  ...
  try {
    ...
    assert.equal(status.version.version, "0.1.9");
...
test("reports update status from public version metadata", async () => {
  ...
  const manifestServer = createHttpServer((req, res) => {
    ...
    res.end(JSON.stringify({
      ...
      showTutorialWhenUpdatingFromBefore: "0.1.9",
    }));
  });
  ...
  try {
    ...
    assert.equal(status.current.version, "0.1.9");
    assert.equal(status.latest?.version, "0.1.10");
